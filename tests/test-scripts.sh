...
# shellcheck disable=SC2086,SC2034  # $TEST_TMPDIR is always a mktemp path without spaces
set -euo pipefail
...
for script in pre-commit pre-push install-hooks.sh check-pre-commit.sh check-pre-push.sh; do
	if [ -f "$REPO_ROOT/scripts/$script" ] && bash -n "$REPO_ROOT/scripts/$script" 2>/dev/null; then
		pass "$script syntax valid"
	else
		fail "$script syntax valid" "bash -n failed or file missing"
	fi
done
...
# 5j. Ruff style findings stay low severity; correctness findings can stay high
RUFF_INPUT='[
  {"filename":"scripts/orchestrate.py","location":{"row":10},"message":"Line too long (121 > 88)","code":"E501"},
  {"filename":"scripts/orchestrate.py","location":{"row":20},"message":"Undefined name foo","code":"F821"}
]'
echo "$RUFF_INPUT" >"$TEST_TMPDIR/test-ruff-input.json"
RUFF_NORM=$(bash -c '
  eval "$(sed -n "/^normalize_ruff/,/^}/p" "'"$SCRIPTS/run-scans.sh"'")"
  normalize_ruff < "'"$TEST_TMPDIR/test-ruff-input.json"'"
' 2>/dev/null || echo '[]')
assert_json_valid "ruff normalizer produces valid JSON" "$RUFF_NORM"
assert_json_field "ruff E501 maps to low severity" "$RUFF_NORM" \
	"any(f['evidence']=='E501' and f['severity']=='low' for f in d)"
assert_json_field "ruff F821 maps to high severity" "$RUFF_NORM" \
...
if [ -f "$REPO_ROOT/justfile" ] && grep -q '^pre-commit:' "$REPO_ROOT/justfile" &&
	...
	grep -q '^check:' "$REPO_ROOT/justfile"; then
	pass "justfile exposes pre-commit, pre-push, and check targets"
else
	fail "justfile exposes pre-commit, pre-push, and check targets" "target missing"
fi
