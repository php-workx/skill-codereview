...
|---------|--------|-------|
| F0c: code_intel.py | Implemented | 9 subcommands: complexity, functions, imports, exports, callers, patterns, graph (repo search, co-change, depth-2, cache, semantic), format-diff (with --expand-context), setup (with --install, --non-interactive) |
| F0b: enrich-findings.py | Implemented | 11-step pipeline, llm_prompt, code-intel boost |
| F1: Prescan | Implemented | 8 pattern checkers (P-SEC through P-STUB), L1-L4 completeness, imports code_intel.py for shared parsing, P-UNWIRED functional |
| F2: Domain Checklists | Implemented | SQL, LLM, concurrency checklists |
| F12: Cross-File Planner | Implemented | LLM planner via cross_file_planner.py (Option A wrapper), deterministic fallback, 5 search categories, per-explorer routing, 5k token budget |
| F13: REVIEW.md Directives | Implemented | Skip patterns, always-check directives |
| F15: Path-Based Instructions | Implemented | Per-path review instructions with trust boundary tags |
| Graph: repo-wide search | Implemented | Step 2: grep for callers of changed symbols, "calls" edges, 20 results/symbol cap, timeout guards |
| Graph: co-change frequency | Implemented | Step 4: git log co-change analysis, frequency >= 3, max 50 edges |
| Graph: --depth 2 | Implemented | Two-hop traversal, hop_distance field, 500 node cap |
| Graph: --cache | Implemented | Incremental indexing, mtime-based invalidation, repo-hash keyed |
| Graph: --semantic | Implemented | model2vec/onnxruntime + sqlite-vec, graceful degradation, cosine similarity edges |
| format-diff wiring | Implemented | Explorers receive before/after blocks, budget-aware --expand-context, raw diff preserved for tools |
| PromptBudgetExceeded handler | Implemented | prepare() catches and writes error launch.json |
| Adaptive expert selection | Implemented | Structural detection from code_intel exports, fallback to diff patterns |
...
**Marker file for CI:**
Add `.agents/codereview/` to `.gitignore`. The marker file reflects machine state and should not be committed.
...
- name: Skip codereview setup
  run: mkdir -p .agents/codereview && touch .agents/codereview/setup-complete
```
