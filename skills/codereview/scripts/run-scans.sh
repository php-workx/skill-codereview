...
# Script directory (for locating bundled rules)
...
get_version() {
	...
	sg | ast-grep) sg --version 2>/dev/null | head -1 ;;
    ...
    [(.[] // empty) |
      {
        ...
        summary: (.message // "ast-grep issue"),
        severity: (
          if (.severity // "" | test("error"; "i")) then "high"
          elif (.severity // "" | test("warning"; "i")) then "medium"
          elif (.severity // "" | test("hint|info"; "i")) then "low"
          else "medium"
          end
        ),
        confidence: 1.0,
        evidence: (
          (.ruleId // "") +
          (if (.note // "" | test("\\[CWE-[0-9]+\\]")) then
            " " + (.note | capture("(?<cwe>\\[CWE-[0-9]+\\])") | .cwe)
          else "" end)
        ),
        pass: (
          if (.ruleId // "" | test("security|vuln|injection|crypto|auth|xss|xxe|ssrf|idor"; "i")) then "security"
          elif (.note // "" | test("\\[CWE-[0-9]+\\]")) then "security"
          elif (.ruleId // "" | test("correctness|bug|error|null-deref"; "i")) then "correctness"
          elif (.ruleId // "" | test("performance|complexity"; "i")) then "performance"
          else "maintainability"
          end
        ),
        source: "deterministic",
    ...
    [(.[] // empty) |
      {
        ...
        severity: (
          if (.code // "" | test("^E9|^F7|^F82[123]$")) then "high"
          elif (.code // "" | test("^F")) then "medium"
          elif (.code // "" | test("^[EW]")) then "low"
          else "low"
          end
        ),
        confidence: 1.0,
...
# Pinned to verified commit SHA for supply-chain safety.
# Last verified: 2025-03-27 (HEAD at time of pinning)
AST_GREP_PINNED_SHA="73120109bf45c284d0cd8a37bdd7082e80e92e87"
if command -v sg >/dev/null 2>&1; then
	AST_GREP_RULES="${SCRIPT_DIR}/../rules/ast-grep-essentials"
	if [ ! -d "$AST_GREP_RULES" ]; then
		log "ast-grep: rules not found, cloning..."
	...
	if [ -n "$AST_GREP_RULES" ] && [ -f "$AST_GREP_RULES/sgconfig.yml" ]; then
		(
			run_tool ast_grep 30 sg scan --config "$AST_GREP_RULES/sgconfig.yml" --json=compact -- "${FILES[@]}"
		) &
		TIER1_PIDS="$TIER1_PIDS $!"
	fi
else
	log "ast-grep: not installed"
	record_status "ast_grep" "not_installed" "null" 0 "cargo install ast-grep"
fi
...
if command -v trivy >/dev/null 2>&1; then
	...
	if [ ${#TRIVY_TARGETS[@]} -gt 0 ]; then
		(
			...
			# trivy does not support -- before positional args; filenames are validated against MANIFEST_PATTERNS above
...
if [ -f .pre-commit-config.yaml ] && command -v pre-commit >/dev/null 2>&1; then
	(
		# pre-commit --files consumes all remaining args as filenames; -- is not supported after --files
