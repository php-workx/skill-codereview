...
if [ -n "$VERDICT" ]; then
	...
	*)
		echo "FAIL: Invalid verdict value: $VERDICT (must be PASS/WARN/FAIL)"
		ERRORS=$((ERRORS + 1))
		;;
	esac
fi
...
BAD_TOOL_STATUS=$(jq '[.tool_status[]? | select((.status // "") | IN("ran","skipped","failed","not_installed","sandbox_blocked","timeout","partial","removed") | not)] | length' "$FINDINGS")
...
if [ "$(jq 'has("review_mode")' "$FINDINGS")" = "true" ]; then
	...
	*)
		echo "FAIL: Invalid review_mode: $REVIEW_MODE (must be standard/chunked)"
		ERRORS=$((ERRORS + 1))
		;;
	esac
